...
exports.getApplicationsForJob = async (req, res) => {
  try {
    ...
    const applications = await Applications.find({ jobId })
      .populate("candidateId", "name email phone about image linkedin github")
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
    ...
    const formattedApplications = applications.map((app) => ({
      ...
      candidate: {
        ...
        about: app.candidateId.about,
        image: app.candidateId.image,
        linkedin: app.candidateId.linkedin,
        github: app.candidateId.github,
      },
...
exports.myapplications = async (req, res) => {
  try {
    ...
    const applications = await Applications.find({ candidateId })
      .populate("jobId", "title company location job_type active")
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
...
exports.downloadResume = async (req, res) => {
  try {
    ...
    const isCandidate = application.candidateId.toString() === req.user.userId;
    ...
    const absolutePath = path.resolve(application.resume.path);
    return res.download(absolutePath, application.resume.originalName || "resume.pdf");
  } catch (error) {
